class CompactPowerCard extends (window.LitElement ||
  ...
  static getConfigForm() {
    return {
      ...
      computeLabel: (schema) => {
        ...
        if (schema.name === "remove_glow_effects") return "Remove Glow Effects?";
  ...
  static getStubConfig() {
    return {
      type: "custom:compact-power-card-cf",
      curved_lines: true,
      curve_factor: 1,
      entities: {
  ...
  static get styles() {
    ...
    return css`
      ...
      :host(.no-grid) #line-pv-grid,
      :host(.no-grid) #line-grid-home,
      :host(.no-grid) #arc-grid-battery,
      :host(.no-grid) #dot-pv-grid,
      :host(.no-grid) #dot-grid-home,
      :host(.no-grid) #dot-grid-battery,
      :host(.no-grid) .grid-marker,
      ha-card.no-grid #line-pv-grid,
      ha-card.no-grid #line-grid-home,
      ha-card.no-grid #arc-grid-battery,
      ha-card.no-grid #dot-pv-grid,
      ha-card.no-grid #dot-grid-home,
      ha-card.no-grid #dot-grid-battery,
      ha-card.no-grid .grid-marker {
        display: none;
      }

      :host(.no-home) .home-marker,
      :host(.no-home) #line-home-battery,
      :host(.no-home) #dot-battery-home,
      ha-card.no-home .home-marker,
      ha-card.no-home #line-home-battery,
      ha-card.no-home #dot-battery-home {
        display: none;
      }
    `;
  }
  ...
  updated(changedProps) {
    ...
    // Ensure flows are updated after DOM is rendered
    this._updateFlows();
  }
  ...
  _allowGlowEffects() {
    return !this._config?.remove_glow_effects && !this._isLightTheme();
  }
  ...
  _updateFlows() {
    ...
    const batteryLabelsSource = Array.isArray(this._config?.entities?.battery)
      ...
      : this._config?.entities?.battery_labels || batteryCfg?.labels;
    const batteryLabels = this._normalizeLabels(batteryLabelsSource);
    const pvUnit =
    ...
    // Check if home exists to position battery correctly for dot animations
    const hasHome =
      this._config?.entities &&
      Object.prototype.hasOwnProperty.call(this._config.entities, "home") &&
      Boolean(homeCfg?.entity);
    // When home doesn't exist, move battery to the bottom center (where home would be)
    const batteryNode = !hasHome
      ? { x: homeCenterX, y: homeLineEndY }
      : { x: gridLineEndX, y: gridNodeY };
    ...
    // DISABLED: PV → Home flow (we only want PV → Battery)
    // if (pvToHome > threshold)
    //   active["pv-home"] = { geom: geom["pv-home"], magnitude: pvToHome, color: pvColor };

    // ENABLED: PV → Battery - ALWAYS SHOW IF PV > 0
    if (pvFlow > 0)
      active["pv-battery"] = { geom: geom["pv-battery"], magnitude: pvFlow, color: pvColor };

    // ENABLED: Grid/Charger → Battery - ALWAYS SHOW IF GRID IMPORT > 0
    if (gridImport > 0)
      active["grid-battery"] = { geom: geom["grid-battery"], magnitude: gridImport, color: gridColor };

    // ENABLED: Battery → Home - ALWAYS SHOW IF BATTERY DISCHARGE > 0
    if (battDischarge > 0)
      active["battery-home"] = { geom: geom["battery-home"], magnitude: battDischarge, color: batteryColor };

    // DISABLED: Battery → Grid flow
    ...
    const allNames = [
      // Only the 3 flows we want to show:
      "pv-battery",      // PV → Battery
      "grid-battery",    // Grid/Charger → Battery
  ...
  render() {
    ...
    const hasGrid =
      this._config?.entities &&
      Object.prototype.hasOwnProperty.call(this._config.entities, "grid") &&
      Boolean(gridCfg?.entity);
    const homeCfg = this._getEntityConfig("home");
    const hasHome =
      this._config?.entities &&
      Object.prototype.hasOwnProperty.call(this._config.entities, "home") &&
    ...
    const pvGridTurnRadius = 8; // slightly larger radius for the grid→PV corner
    const pvBatteryStartX = homeCenterX; // CENTER - PV→Battery starts at center
    const pvBatteryEndY = gridNodeY - 10; // lift PV→Battery end 10px above grid/battery baseline
    const gridHomeStartY = gridNodeY + 10; // drop grid→home start 10px below grid/battery baseline
    const gridHomeEndX = homeCenterX - 10; // end 10px left of home center
    const batteryHomeStartY = gridNodeY + 10; // drop battery→home start 10px below grid/battery baseline
    const batteryHomeEndX = homeCenterX; // CENTER - Battery→Home ends at center
    const pvNode = { x: pvCenterX, y: pvNodeY };
    const gridNode = { x: gridLineStartX, y: gridNodeY };
    // When home doesn't exist, move battery to the bottom center (where home would be)
    const batteryNode = !hasHome
    ...
    // When home doesn't exist, center battery icon at bottom; otherwise keep it on the right
    const batteryIconX = !hasHome ? homeCenterX : gridLineEndX + iconOffset;
    ...
    const batteryIconDefault = Number.isFinite(batterySocPrimary)
    ...
    // Icon configuration with defaults
    const pvIcon = pvCfg?.icon || "mdi:solar-panel";
    const gridIcon = gridCfg?.icon || "mdi:transmission-tower";
    const homeIcon = homeCfg?.icon || "mdi:home";
    const batteryIcon = batteryCfg?.icon || batteryIconDefault;
    ...
    this.classList.toggle("no-grid", !hasGrid);
    this.classList.toggle("no-home", !hasHome);
    this.classList.toggle("has-pv-labels", pvLabels.length > 0);
    this.classList.toggle("has-grid-labels", gridLabels.length > 0);
    this.classList.toggle("has-battery-labels", batteryLabels.length > 0);
    ...
    // When home doesn't exist, position battery icon at bottom; otherwise at grid/battery baseline
    const batteryIconTop = !hasHome ? homeAnchorY : gridNodeY + 9;
    ...
    // When home doesn't exist, position battery labels left/right of battery icon; otherwise above on right
    const batteryLabelPositions = !hasHome
      ? [
          { xPct: ((batteryIconX - 60) / baseWidth) * 100, yPx: homeAnchorY },
          { xPct: ((batteryIconX + 60) / baseWidth) * 100, yPx: homeAnchorY },
        ]
      : [
    ...
    // PV→Battery: when home doesn't exist, go straight down to battery at bottom (stop 15px before icon)
    const pvBatteryPath = !hasHome
      ? `M${pvNode.x} ${pvNode.y} V${batteryNode.y - 25}`
      : makeCornerPath(pvBatteryStartX, pvNode.y, batteryNode.x, pvBatteryEndY, 0, "V");
    ...
    // Grid→Battery: when home doesn't exist, go from grid to battery at bottom; otherwise straight horizontal
    const gridBatteryPath = !hasHome
      ? makeCornerPath(gridNode.x, gridNodeY, batteryNode.x, batteryNode.y, 1, "H")
    ...
    return html`
      <ha-card class="${[
        ...
        <div class="canvas">
          ...
          <!-- Flow lines - only showing PV→Battery, Battery→Home, Grid→Battery -->
          <!-- Hidden: line-pv-grid -->
          <!-- Hidden: line-pv-home -->
          <path id="line-pv-battery" class="flow-line" fill="none" d="${pvBatteryPath}" />
          <!-- Hidden: line-grid-home -->
          ...
          <div class="overlay">
            ...
            ${pvLabelItems.map(
              (lbl) => html`<div class="overlay-item" style="left:${lbl.leftPct}%; top:${lbl.topPct}%;">
                <div class="aux-marker clickable" @click=${() => this._openMoreInfo(lbl.entity || null)}>
                  <ha-icon icon="${lbl.icon}" style="gap: 0px; color:${lbl.color}; opacity:1; --mdc-icon-size: calc(18px * var(--cpc-scale, 1));"></ha-icon>
            ...
            ${gridLabelItems.map(
              (lbl) => html`<div class="overlay-item anchor-left" style="left:${lbl.xPct}%; top:${lbl.yPx}px;">
                <div class="aux-marker clickable" style="flex-direction: row; gap: 4px;" @click=${() => this._openMoreInfo(lbl.entity || null)}>
                  <ha-icon icon="${lbl.icon}" style="color:${lbl.color}; opacity:1; --mdc-icon-size: calc(16px * var(--cpc-scale, 1));"></ha-icon>
            ...
            ${hasBattery
              ? batteryLabelItems.map(
                  (lbl) => html`<div class="overlay-item ${!hasHome ? "" : "anchor-right"} battery-label" style="${!hasHome ? "" : "margin-right: 10px;"} left:${lbl.xPct}%; top:${lbl.yPx}px;">
                    <div class="aux-marker clickable" style="flex-direction: row; gap: 4px;" @click=${() => this._openMoreInfo(lbl.entity || null)}>
                      <div class="aux-label" style="color:${lbl.color}; opacity:${lbl.hidden ? 0.35 : lbl.opacity};">${renderValue(lbl.val)}</div>
                      <ha-icon icon="${lbl.icon}" style="color:${lbl.color}; opacity:1; --mdc-icon-size: calc(16px * var(--cpc-scale, 1));"></ha-icon>
                    </div>
            ...
            <div class="overlay-item pv-section" style="left:${(sx(256)/baseWidth)*100}%; top:${pctBaseY(24)}%;">
              <div class="node-marker pv-marker clickable" @click=${() => this._openMoreInfo(pvCfg.entity)}>
                ...
                <ha-icon icon="${pvIcon}" style="color:${pvColor}; opacity:1; filter:${this._allowGlowEffects() && pvNumeric !== 0 ? `drop-shadow(0 0 10px ${pvColor})` : "none"};"></ha-icon>
              </div>
            </div>
            <div class="overlay-item anchor-left" style="left:${(gridIconX/baseWidth)*100}%; top:${gridIconTop}px;">
              <div class="node-marker grid-marker left clickable" @click=${() => this._openMoreInfo(gridCfg.entity)}>
                <ha-icon icon="${gridIcon}" style="color:${gridColor}; opacity:1; filter:${this._allowGlowEffects() && gridNumeric !== 0 ? `drop-shadow(0 0 10px ${gridColor})` : "none"};"></ha-icon>
                <div
            ...
            ${enableDevicePowerLines && hasDeviceSources && deviceUsageActive
              ...
              : ""}
            ${hasBattery
              ? html`<div class="overlay-item ${!hasHome ? "" : "anchor-right"} battery-section" style="left:${(batteryIconX/baseWidth)*100}%; top:${batteryIconTop}px;">
                  <div class="node-marker battery-marker ${!hasHome ? "" : "right"} ${batteryDetails.length ? "" : "clickable"}" @click=${() => {
                    if (!batteryDetails.length) this._openMoreInfo(batteryCfg.entity);
                  }}>
                    <ha-icon icon="${batteryIcon}" style="color:${batteryColor}; opacity:${batteryIconOpacity}; filter:${this._allowGlowEffects() && battNumericW !== 0 ? `drop-shadow(0 0 10px ${batteryColor})` : "none"};"></ha-icon>
                    <div
                      class="node-label ${!hasHome ? "" : "right"} ${batteryLabelFlicker ? "label-flicker" : ""}"
                    ...
                    >
                      ${battArrow && !batteryLabelHidden
                        ...
                        : ""}
                      <span style="opacity:1;">${battValDisplay}</span>
                    </div>
                  </div>
                </div>`
              : ""}
              ...
              (src) => html`<div class="overlay-item" style="left:${src.leftPct}%; top:${src.topPct}%; transform: translate(-50%, -50%);">
                <div class="aux-marker">
                  ...
                  >
                    <ha-icon
                      icon="${src.icon}"
                      style="color:${src.color}; opacity:1; filter:${this._allowGlowEffects() && src.numeric !== 0 ? `drop-shadow(0 0 10px ${src.color})` : "none"};"
                    ></ha-icon>
                  </span>
                  <div
...
if (window?.customCards) {
  const exists = window.customCards.some((c) => c.type === "compact-power-card-cf");
  if (!exists) {
    window.customCards.push({
      type: "compact-power-card-cf",
      name: "Compact Power Card CF",
      preview: true,
      description: "Simplified power flow card showing PV→Battery, Grid→Battery, and Battery→Home flows.",
      documentationURL: "https://github.com/craigusus/Compact-Power-Card-CF/blob/main/README.md",
    });
  }
} else if (window) {
  window.customCards = [
